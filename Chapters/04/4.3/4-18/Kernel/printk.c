...
int ColorPrintk(unsigned int FRColor, unsigned int BKColor, const char * fmt,...)
{
	...
	for (count = 0; count < i || line; count ++)
	{
		...
		{
			line = ((Position.XPosition + 8) & ~(8 - 1)) - Position.XPosition;
			
LabelTab:
			line --;
			
			putchar(Position.FBAddress, Position.XResolution, Position.XPosition * Position.XCharSize, Position.YPosition * Position.YCharSize, FRColor, BKColor, ' ');
		...
		{
			putchar(Position.FBAddress, Position.XResolution, Position.XPosition * Position.XCharSize, Position.YPosition * Position.YCharSize, FRColor, BKColor, (unsigned char) *(buffer + count));
